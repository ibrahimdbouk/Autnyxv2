...
class AnomalySetting extends Model
{
    ...
    const RULES = [
        ...
        // ── Tenant-defined (W10) ──────────────────────────────────────────────
        // One switch for every rule the tenant wrote (Root Cause → Custom
        // rules); each rule is its own subject (`custom:<key>`) and carries its
        // own label, severity and value type.
        'custom_rule' => [
            'label'              => 'Custom Rule',
            'description'        => 'A rule your team defined as a formula on store × SKU data (Root Cause → Custom rules).',
            'severity'           => 'medium',
